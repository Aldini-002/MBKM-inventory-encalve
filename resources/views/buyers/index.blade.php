@extends('layouts.table') @section('content')
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right"><a
                                                href="/buyers/create{{ str_replace('/buyers', '', request()->getRequestUri()) }}"
                                ...
                                <tbody>
                                    @foreach ($buyers as $data)
                                        <tr>
                                            ...
                                            <td class="text-right">{{ count($data->stock_outs) }}</td>
                                            <td class="project-actions text-right">
                                                <a class="btn btn-primary btn-sm"
                                                    href="/buyers/{{ $data->id }}{{ str_replace('/buyers', '', request()->getRequestUri()) }}">
                                                    <i class="fas fa-folder"> </i>
                                                    Lihat
                                                </a>
                                                <a class="btn btn-info btn-sm"
                                                    href="/buyers/{{ $data->id }}/edit{{ str_replace('/buyers', '', request()->getRequestUri()) }}">
    ...
    <form action="/buyers" method="get" id="form-order-by-name">
    ...
    <form action="/buyers" method="get" id="form-order-by-date">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
        <input type="hidden" name="show" value="{{ request('show') ?? 5 }}" />
