@extends('layouts.table') @section('content')
                ...
                <div class="col-12">
                    <div class="card card-dark card-outline">
                        <div class="card-header">
                            <h3 class="card-title">Tabel Daftar Kategori</h3>
                            ...
                            <table class="table table-hover text-nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right"><a href="/categories/create"
                                            ...
                                            <td class="project-actions text-right">
                                                <div class="d-flex justify-content-end">
                                                    ...
                                                    <form action="/categories/{{ $data->id }}" method="post">
                                                        ...
                                                        @method('delete')
                                                        <button
                                                            onclick="return confirm('PERHATIAN!!!\nSemua furniture yang menggunakan kategori {{ $data->name }} juga akan terhapus.')"
    ...
    <form action="/categories" method="get" id="form-order-by-name">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
        <input type="hidden" name="orderBy" value="name" />
        <input type="hidden" name="order" value="{{ request('order') == 'desc' ? 'asc' : 'desc' }}" />
    </form>

    <form action="/categories" method="get" id="form-order-by-date">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
