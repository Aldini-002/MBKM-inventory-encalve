@extends('layouts.table') @section('content')
                ...
                <div class="col-12">
                    <div class="card card-dark card-outline">
                        <div class="card-header">
                            <h3 class="card-title">Tabel Daftar Material</h3>
                            ...
                            <table class="table table-hover text-nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right"><a href="/materials/create" class="btn btn-primary btn-sm"><i
                                ...
                                <tbody>
                                    @foreach ($materials as $data)
                                        <tr>
                                            ...
                                            <td class="text-right">
                                                {{ count($furniture_materials->where('material_id', $data->id)) }}</td>
                                            ...
                                            <td class="project-actions text-right">
                                                <div class="d-flex justify-content-end">
                                                    ...
                                                    <form action="/materials/{{ $data->id }}" method="post">
                                                        ...
                                                        @method('delete')
                                                        <button
                                                            onclick="return confirm('PERHATIAN!!!\nSemua furniture yang menggunakan material {{ $data->name }} juga akan terhapus.')"
    ...
    <form action="/materials" method="get" id="form-order-by-name">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
        <input type="hidden" name="orderBy" value="name" />
        <input type="hidden" name="order" value="{{ request('order') == 'desc' ? 'asc' : 'desc' }}" />
    </form>

    <form action="/materials" method="get" id="form-order-by-date">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
