@extends('layouts.table') @section('content')
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-right">
                                            <div class="d-flex justify-content-end">
                                                <form action="/stockinselects/cancel" method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="btn btn-danger btn-sm mr-1"><i
                                                            class="fas fa-solid fa-x"></i> Batal</button>
                                                </form>
                                                <a href="/stockins/create" class="btn btn-primary btn-sm"><i
                                    ...
                                    @foreach ($stockinselects as $data)
                                        <tr>
                                            <td>
                                                <div class="d-flex">
                                                    ...
                                                    <div class="text-capitalize ml-3">
                                                        {{ $data->furniture->name }}
                                                        <div class="text-sm text-gray">{{ $data->furniture->code }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="project-actions text-right">
                                                <form action="/stockinselects/{{ $data->id }}" method="post">
                                                    @csrf
                                                    @method('delete')
                            ...
                            <table class="table table-hover text-nowrap">
                                ...
                                <tbody>
                                    @foreach ($furnitures as $data)
                                        <tr class="{{ $data->stock_in_select ? 'd-none' : '' }}">
                                            <td>
                                                <div class="d-flex">
                                                    ...
                                                    <div class="text-capitalize ml-3">
                                                        {{ $data->name }}
                                                        <div class="text-sm text-gray">{{ $data->code }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="project-actions text-right">
                                                <form action="/stockinselects" method="post">
                                                    @csrf
                                                    <input type="hidden" name="furniture_id"
                                                        value="{{ $data->id }}">
    ...
    <form action="/stockinselects" method="get" id="form-order-by-name">
    ...
    <form action="/stockinselects" method="get" id="form-order-by-date">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
        <input type="hidden" name="category" value="{{ request('category') ?? '' }}" />
        <input type="hidden" name="material" value="{{ request('material') ?? '' }}" />
        <input type="hidden" name="show" value="{{ request('show') ?? 5 }}" />
