@extends('layouts.table') @section('content')
                ...
                <div class="col-12">
                    <div class="card card-dark card-outline">
                        <div class="card-header">
                            <h3 class="card-title">Tabel Daftar Furnitur</h3>

                            <form action="/supliers">
                                <div class="card-tools d-flex justify-content-end">
                                    ...
                                    <div class="input-group input-group-sm" style="width: 300px">
                                        <input type="text" name="search" class="form-control float-right"
                                            placeholder="Cari nama/akun/telepon" value="{{ request('search') }}"
                                            autocomplete="off" />
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right"><a
                                                href="/supliers/create{{ str_replace('/supliers', '', request()->getRequestUri()) }}"
                                ...
                                <tbody>
                                    @foreach ($supliers as $data)
                                        <tr>
                                            ...
                                            <td class="text-right">{{ count($data->stock_ins) }}</td>
                                            <td class="project-actions text-right">
                                                <a class="btn btn-primary btn-sm"
                                                    href="/supliers/{{ $data->id }}{{ str_replace('/supliers', '', request()->getRequestUri()) }}">
                                                    <i class="fas fa-folder"> </i>
                                                    Lihat
                                                </a>
                                                <a class="btn btn-info btn-sm"
                                                    href="/supliers/{{ $data->id }}/edit{{ str_replace('/supliers', '', request()->getRequestUri()) }}">
    ...
    <form action="/supliers" method="get" id="form-order-by-name">
    ...
    <form action="/supliers" method="get" id="form-order-by-date">
        <input type="hidden" name="search" value="{{ request('search') ?? '' }}" />
        <input type="hidden" name="show" value="{{ request('show') ?? 5 }}" />
